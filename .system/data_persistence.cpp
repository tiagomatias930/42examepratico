...
// ==> Armazenar dados do exame em um arquivo
...
// ==> Restaurar uma versão antiga do exame
void exam::restore_data(void)
{
    ...
    {
        std::cout << "Erro: não é possível abrir o backup" << std::endl;
    }
    ...
    if (backup.end_time > time(0))
    {
        ...
        std::cout << "    EXATUAL: " << LIME << backup.current_ex->get_name() << RESET << std::endl;
        std::cout << RED << "    " << remaining_time(backup.end_time) << std::endl;
        std::cout << RED << "           restante" << std::endl
                  << std::endl;

        std::cout << WHITE << BOLD << "            1" << RESET << std::endl;
        std::cout << WHITE << BOLD << "    |" << LIME << BOLD << "  RESTAURAR EXAME  " << WHITE << BOLD << "|" << RESET << BOLD << std::endl
                  << "     \\ ------------ /" << std::endl
                  << std::endl;
        std::cout << WHITE << BOLD << "            2" << RESET << std::endl;
        std::cout << WHITE << BOLD << "     | " << RED << BOLD << " APAGAR EXAME" << WHITE << BOLD << "  |" << RESET << BOLD << std::endl
                  << "      \\ ---------- /" << std::endl
                  << std::endl
                  << std::endl;
        std::cout << "            ";
        ...
        if (answer == "1")
        {
            std::cin.ignore();
            std::cout << std::endl
                      << "   Restaurando token de exame..." << RESET << std::endl
                      << std::endl;
        ...
        {
            std::cout << "   Token de exame deletado" << std::endl;
            std::cin.ignore();
...
// ==> Carregar arquivo .settings em bool setting_dse
...
// ==> Salvar bool setting_dse no arquivo .settings
